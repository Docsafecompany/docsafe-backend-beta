...
async function cleanOrphanedRelationships(zip) {
  ...
  for (const relsPath of relsFiles) {
    ...
    if (removedHere > 0) {
      ...
      const decl = xml.startsWith("<?xml") ? (xml.match(/^\<\?xml[^>]*\?>\s*/i)?.[0] || "") : "";
      ...
      console.log(`[PPTX:ZIP] Cleaned orphan rels: ${relsPath} removed=${removedHere} kept=${kept.length}`);
    }
  }
...
async function cleanContentTypes(zip) {
  ...
  console.log(`[PPTX:ZIP] Cleaned [Content_Types].xml overrides removed=${removed} kept=${keptOverrides.length}`);
...
export async function correctDOCXText(buffer, correctFn, options = {}) {
  ...
  const detected = Array.isArray(options.detectedSpellingErrors) ? options.detectedSpellingErrors : [];
  const approved = Array.isArray(options.spellingErrors) ? options.spellingErrors : [];
  const spellingErrors = normalizeApprovedList(detected.length ? detected : approved, approved);
