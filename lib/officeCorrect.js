...
// VERSION 5.1 — ANCHORED & SAFE (DOCX + PPTX + XLSX)
// ✔ Context-aware correction
// ✔ Multi-run safe strategy
// ✔ Fixed: allTextStart/allTextEnd now updated after modifications
// ✔ Added: XML validation before write
// ✔ Added: Skip problematic corrections
...
function isXmlSafe(text) {
  // Check if the text would break XML when encoded
  const encoded = encode(text);
  // Should not contain unencoded < or > after encoding
...
// ---------------- Apply corrections in segmented XML ----------------
function applyCorrectionsToSegments(xml, tagRegex, spellingErrors, stats) {
  ...
  for (const err of spellingErrors || []) {
    ...
    if (overlapped.length === 1) {
      ...
      // ✅ FIX: Update BOTH xml indices AND text indices for subsequent segments
      ...
      // Update allText to reflect the change
    ...
    // Calculate total text difference for multi-segment
    ...
    for (let n = 0; n < overlapped.length; n++) {
      ...
      modified = modified.slice(0, s.xmlStart + cumulativeXmlDiff) + nextTag + modified.slice(s.xmlEnd + cumulativeXmlDiff);

      const xmlDiff = nextTag.length - s.full.length;
      cumulativeXmlDiff += xmlDiff;

      s.full = nextTag;
      s.raw = encode(nextText);
      s.text = nextText;
      s.xmlStart += cumulativeXmlDiff - xmlDiff;
      s.xmlEnd = s.xmlStart + nextTag.length;
    }

    // ✅ FIX: Update indices for all segments AFTER the last overlapped segment
    const lastOverlappedIndex = overlapped[overlapped.length - 1].i;
  ...
  if (skippedCount > 0) {
    console.log(`[CORRECT] Applied: ${appliedCount}, Skipped (unsafe): ${skippedCount}`);
  }
...
export async function correctDOCXText(buffer, correctFn, options = {}) {
  ...
  console.log(`[DOCX] Processing ${targets.filter(t => zip.file(t)).length} files, ${spellingErrors.length} corrections`);
  ...
  for (const p of targets) {
    ...
    if (spellingErrors.length) {
      xml = applyCorrectionsToSegments(xml, /<w:t[^>]*>([\s\S]*?)<\/w:t>/g, spellingErrors, stats);
    }
...
export async function correctPPTXText(buffer, correctFn, options = {}) {
  ...
  for (const p of targets) {
    ...
    if (spellingErrors.length) {
      xml = applyCorrectionsToSegments(xml, /<a:t>([\s\S]*?)<\/a:t>/g, spellingErrors, stats);
    }
...
export async function correctXLSXText(buffer, correctFn, options = {}) {
  ...
  console.log(`[XLSX] Processing ${targets.filter(t => zip.file(t)).length} files, ${spellingErrors.length} corrections`);
  ...
  for (const p of targets) {
    ...
    if (spellingErrors.length) {
      xml = applyCorrectionsToSegments(xml, /<t[^>]*>([\s\S]*?)<\/t>/g, spellingErrors, stats);
    }
