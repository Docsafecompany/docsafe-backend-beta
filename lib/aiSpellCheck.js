// lib/aiSpellCheck.js
// VERSION 7.1 - High-recall AI proofreading with overlap + offsets (start/end)
...
// ✅ PATCH 7.1:
//   - preDetectSpacingIssues(): detect multiple spaces + fragmented words BEFORE AI
//   - merge pre-detect + AI errors with global offsets + dedup
...
/**
 * ✅ NEW: pre-detect spacing issues (no AI)
...
export async function checkSpellingWithAI(text) {
  console.log("📝 AI spell-check VERSION 7.1 starting...");
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    console.log(
      `[SPELLCHECK] Chunk ${i + 1}/${chunks.length} candidates: ${candidates.length}`
    );
