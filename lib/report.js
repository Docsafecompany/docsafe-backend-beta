...
export function buildReportHtmlDetailed({ filename, ext, policy, cleaning, correction, analysis, spellingErrors = [] }) {
  ...
  // Récupérer le score depuis analysis ou calculer un défaut
  const score = analysis?.summary?.riskScore ?? 100;
  const riskLevel = analysis?.summary?.riskLevel ?? 'safe';
  const totalIssues = analysis?.summary?.totalIssues ?? 0;
  const criticalIssues = analysis?.summary?.criticalIssues ?? 0;
  const recommendations = analysis?.summary?.recommendations || [];
  
  // Récupérer les spellingErrors depuis plusieurs sources possibles
  const allSpellingErrors = spellingErrors.length > 0 
    ? spellingErrors 
    : (analysis?.detections?.spellingErrors || []);
  ...
  // ====== GÉNÉRATION DES LIGNES DE CORRECTIONS ======
  // Support multiple formats de données
  let correctionRows = '';
  let correctionsCount = 0;
  
  // Méthode 1: correction.examples avec {before, after}
  if (correction?.examples?.length > 0) {
    correctionRows = correction.examples.map(item => {
      // Support flexible des noms de propriétés
      const before = item.before || item.error || item.original || item.word || item.text || '';
      const after = item.after || item.correction || item.corrected || item.suggestion || item.replacement || '';
      const type = item.type || item.category || 'spelling';
      
      return `<tr>
        <td class="cell-before">
          <div class="code-block">${escapeHtml(before)}</div>
          <span class="correction-type">${escapeHtml(type)}</span>
        </td>
        <td class="cell-after"><div class="code-block">${escapeHtml(after)}</div></td>
      </tr>`;
    }).join('');
    correctionsCount = correction.examples.length;
  }
  // Méthode 2: spellingErrors directement
  else if (allSpellingErrors.length > 0) {
    correctionRows = allSpellingErrors.map(err => {
      // Support flexible des noms de propriétés
      const before = err.error || err.word || err.original || err.text || err.before || '';
      const after = err.correction || err.suggestion || err.replacement || err.corrected || err.after || '';
      const type = err.type || err.category || 'spelling';
      const context = err.context || '';
      
      return `<tr>
        <td class="cell-before">
          <div class="code-block">${escapeHtml(before)}</div>
          ${context ? `<div class="context-text">${escapeHtml(context.slice(0, 80))}</div>` : ''}
          <span class="correction-type">${escapeHtml(type)}</span>
        </td>
        <td class="cell-after"><div class="code-block">${escapeHtml(after)}</div></td>
      </tr>`;
    }).join('');
    correctionsCount = allSpellingErrors.length;
  }
  
  const hasCorrections = correctionRows.length > 0;
  const displayCount = correction?.changedTextNodes || correctionsCount || 0;
  ...
  <div class="container">
    ...
    <div class="score-card">
      ...
      <div class="score-stats">
        ...
        <div class="score-stat">
          <div class="score-stat-number">${displayCount}</div>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <h2>Spelling & Grammar Corrections</h2>
        <span class="badge">${displayCount} fixed</span>
      </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="empty-state">
          ...
          <p>No spelling or grammar corrections were needed.</p>
        </div>
      </div>
    </div>
    `}
