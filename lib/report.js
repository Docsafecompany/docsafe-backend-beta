...
export function buildReportData({
  ...
  if (formattedRecommendations.length === 0) {
    if (sensitiveDataItems.length > 0) formattedRecommendations.push("Review all flagged sensitive data before sharing externally");
    if (macroItems.length > 0) formattedRecommendations.push("Remove or disable macros for external document sharing");
    if (hiddenContentItems.length > 0) formattedRecommendations.push("Hidden content detected — ensure it was removed before sharing");
  ...
  const derivedClientReady =
    riskSummary?.clientReady ||
    (severityRank(derivedOverallSeverity) >= 3 ? "NO" : "YES");
  ...
  // EXTRA METRICS
  // ============================================================
  const clarityScoreImprovement = allCorrections.length > 0 ? Math.min(25, Math.round(allCorrections.length * 2.5)) : 0;
  ...
  console.log(
    `[REPORT JSON v3.4] beforeScore=${beforeScore}, afterScore=${afterScore}, riskReduction=${riskReduction}%, complianceStatus=${complianceStatus}, corrections=${allCorrections.length}, comments=${commentItems.length}, sensitiveData=${sensitiveDataItems.length}, hiddenDetails=${hiddenContentDetailedCount}, businessRiskObjects=${businessRisks.totalRiskObjects}, businessOverall=${businessRisks.overallSeverity}, clientReady=${businessRisks.clientReady}`
  );
...
export function buildReportHtmlDetailed({
  ...
  const complianceColors = {
    safe: {
      emoji: "✅",
      label: "Ready to Send",
      subtitle: "This document has been cleaned and is now safe to share with clients or external partners.",
      color: "#10b981",
      bg: "#dcfce7",
    },
    attention: {
  ...
  const businessRiskBadge = (sev) => {
    ...
    return `<span class="risk-level" style="background:${color}20;color:${color}">${getRiskLevelEmoji(s)} ${escapeHtml(severityLabel(s))}</span>`;
  };
      ...
      <div style="padding: 0 24px 18px; color: var(--gray-700); font-size: 14px;">
        ...
        ${
          Array.isArray(br.executiveSignals) && br.executiveSignals.length
            ? `<div style="margin-top:10px;"><strong>Executive Signals:</strong> ${escapeHtml(br.executiveSignals.join(" • "))}</div>`
            : ""
        }
      </div>
            ...
            .map((c) => {
              ...
              return `
                <tr>
                  ...
                  <td class="center"><span style="font-family: 'SF Mono', Monaco, monospace;">${escapeHtml(surf)}</span></td>
                </tr>
              `;
            })
            .join("")}
        ...
        <div style="padding: 16px 24px 24px;">
          ...
          <ul class="items-list" style="margin:0;">
            ${br.blockingIssues
              ...
              <li>
                <div class="item-content">
                  <span class="item-label">${escapeHtml(it.label || it.riskCategory || "Issue")} — ${escapeHtml(it.severity || "")}</span>
                  <span class="item-value">${escapeHtml(it.reason || "")}</span>
                  ${
                    it.fixability
                      ? `<div style="margin-top:6px; font-size:12px; color: var(--gray-500);">Fixability: <strong>${escapeHtml(it.fixability)}</strong></div>`
                      : ""
                  }
                </div>
              </li>
            `
              ...
              .join("")}
            ${br.blockingIssues.length > 10 ? `<li class="more-items">... and ${br.blockingIssues.length - 10} more blocking issues</li>` : ""}
          </ul>
        </div>
        `
          : ""
            ...
            <section class="detail-section" style="margin: 0 24px 24px;">
              ...
              <ul class="items-list">
                ${top
                  ...
                  <li>
                    <div class="item-content">
                      <span class="item-label">${businessRiskBadge(it.severity)} &nbsp; ${escapeHtml(it.surface || "")} ${it.location ? `— ${escapeHtml(it.location)}` : ""}</span>
                      <span class="item-value">${escapeHtml(it.reason || "")}</span>
                ...
                `
                  ...
                  .join("")}
                ${c.items.length > 6 ? `<li class="more-items">... and ${c.items.length - 6} more signals</li>` : ""}
              </ul>
            </section>
          `;
        })
