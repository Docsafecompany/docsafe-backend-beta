...
export function buildReportHtmlDetailed({ filename, ext, policy, cleaning, correction, analysis, spellingErrors = [] }) {
  ...
  // Récupérer les spellingErrors depuis analysis si non passé directement
  ...
  // Générer les lignes de corrections depuis correction.examples OU depuis spellingErrors
  let correctionRows = '';
  
  // D'abord essayer correction.examples
  if (correction?.examples?.length > 0) {
    correctionRows = correction.examples.map(({before, after, type}) =>
      `<tr>
        <td class="cell-before">
          <div class="code-block">${escapeHtml(before)}</div>
          ${type ? `<span class="correction-type">${escapeHtml(type)}</span>` : ''}
        </td>
        <td class="cell-after"><div class="code-block">${escapeHtml(after)}</div></td>
      </tr>`
    ).join('');
  }
  // Sinon utiliser spellingErrors directement
  else if (allSpellingErrors.length > 0) {
    correctionRows = allSpellingErrors.map((err) =>
      `<tr>
        <td class="cell-before">
          <div class="code-block">${escapeHtml(err.error)}</div>
          <span class="correction-type">${escapeHtml(err.type || 'spelling')}</span>
        </td>
        <td class="cell-after">
          <div class="code-block">${escapeHtml(err.correction)}</div>
        </td>
      </tr>`
    ).join('');
  }
  ...
  const correctionsCount = correction?.changedTextNodes || correction?.examples?.length || allSpellingErrors.length || 0;
  ...
  <div class="container">
    ...
    <div class="score-card">
      ...
      <div class="score-stats">
        ...
        <div class="score-stat">
          <div class="score-stat-number">${correctionsCount}</div>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <span class="badge">${correctionsCount} fixed</span>
      </div>
