// lib/report.js
// Génère un rapport HTML et JSON structuré pour Qualion-Doc - v3.2 Enterprise-grade
...
// - Ajout labels PDF (layers/js) si fournis par analyzer
// - Le reste inchangé (comments full text, sensitive data non masqué, etc.)
...
export function buildReportData({
  ...
  if (analysis?.detections?.comments && Array.isArray(analysis.detections.comments)) {
    analysis.detections.comments.forEach(c => {
      ...
      else if (c.rawXml) commentText = 'Comment content';
  ...
  if (analysis?.detections?.trackChanges && Array.isArray(analysis.detections.trackChanges)) {
    analysis.detections.trackChanges.forEach(tc => {
      ...
      trackChangesItems.push({
        ...
        value: displayText.length > 120 ? displayText.substring(0, 120) + '...' : displayText
      });
    });
  }

  // 4. Hidden content items - ✅ DÉTAILLÉ (hidden_text_details.items)
  ...
  if (analysis?.detections?.hiddenContent && Array.isArray(analysis.detections.hiddenContent)) {
    analysis.detections.hiddenContent.forEach(hc => {
      ...
      // ✅ CAS IMPORTANT : hidden_text_details
      if (type === 'hidden_text_details' && Array.isArray(hc.items) && hc.items.length > 0) {
        hc.items.forEach((it) => {
          ...
          const value = textShown.length > 240 ? textShown.slice(0, 237) + '...' : textShown;
      ...
      if (hc.content && typeof hc.content === 'string') {
        value = hc.content.length > 240 ? hc.content.substring(0, 240) + '...' : hc.content;
  ...
  if (analysis?.detections?.hiddenContent && Array.isArray(analysis.detections.hiddenContent)) {
    ...
    // ✅ Si on a hidden_text_details -> remonter aussi comme risque (utile dashboard)
  ...
  if (Array.isArray(recommendations)) {
    recommendations.forEach(rec => {
      ...
      else if (rec?.text) formattedRecommendations.push(`${rec.icon || ''} ${rec.text}`);
    });
  }
  ...
  if (formattedRecommendations.length === 0) {
    ...
    if (macroItems.length > 0) formattedRecommendations.push('Consider disabling macros for external document sharing');
    if (hiddenContentItems.length > 0) formattedRecommendations.push('All hidden content has been removed for security');
  ...
  console.log(
    `[REPORT JSON v3.2] Generated: beforeScore=${beforeScore}, afterScore=${afterScore}, riskReduction=${riskReduction}%, complianceStatus=${complianceStatus}, corrections=${allCorrections.length}, comments=${commentItems.length}, sensitiveData=${sensitiveDataItems.length}, hiddenDetails=${hiddenContentDetailedCount}`
  );
