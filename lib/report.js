// lib/report.js
// Qualion-Doc - v3.3 Enterprise-grade (compatible analyzer v3.2)
// Fixes:
...
export function buildReportData({
  ...
  const sensitiveDataRiskLevel = sensitiveDataItems.some((i) => i.label.includes("IBAN") || i.label.includes("SSN") || i.label.includes("CREDIT"))
    ? "critical"
  ...
  const cleaningSummary = {
    ...
    trackChanges: {
      count:
        ...
        (fmt(cleaning?.revisionsAccepted?.deletionsRemoved) + fmt(cleaning?.revisionsAccepted?.insertionsUnwrapped)) ||
        0,
  ...
  if (Array.isArray(analysis?.detections?.hiddenContent)) {
    const criticalHidden = analysis.detections.hiddenContent.filter((h) => h.type === "white_text" || h.type === "vanished_text");
  ...
  console.log(
    `[REPORT JSON v3.3] beforeScore=${beforeScore}, afterScore=${afterScore}, riskReduction=${riskReduction}%, complianceStatus=${complianceStatus}, corrections=${allCorrections.length}, comments=${commentItems.length}, sensitiveData=${sensitiveDataItems.length}, hiddenDetails=${hiddenContentDetailedCount}`
  );
