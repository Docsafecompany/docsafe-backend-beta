// lib/report.js
// Génère un rapport HTML et JSON structuré pour Qualion-Doc - v2.5 avec score before/after et items détaillés
...
export function buildReportData({ 
  ...
  const afterScore = afterRiskScore ?? analysis?.summary?.riskScore ?? Math.min(100, beforeScore + 20);
  ...
  if (analysis?.detections?.comments && Array.isArray(analysis.detections.comments)) {
    analysis.detections.comments.forEach(c => {
      ...
      const content = c.content || c.text || c.comment || '';
      commentItems.push({ 
        label: `By ${author}`, 
        value: content.length > 40 ? content.substring(0, 40) + '...' : content 
      });
    });
  }
  ...
  if (analysis?.detections?.trackChanges && Array.isArray(analysis.detections.trackChanges)) {
    analysis.detections.trackChanges.forEach(tc => {
      ...
      const text = tc.text || tc.content || '';
      trackChangesItems.push({ 
        label: type.charAt(0).toUpperCase() + type.slice(1), 
        value: text.length > 40 ? text.substring(0, 40) + '...' : text 
      });
    });
  }
  ...
  if (analysis?.detections?.hiddenContent && Array.isArray(analysis.detections.hiddenContent)) {
    analysis.detections.hiddenContent.forEach(hc => {
      const type = hc.type || hc.elementType || 'Hidden element';
      const location = hc.location || hc.slide || '';
      hiddenContentItems.push({ label: type, value: location });
    });
  }
  if (analysis?.detections?.hiddenSheets && Array.isArray(analysis.detections.hiddenSheets)) {
    analysis.detections.hiddenSheets.forEach(hs => {
      hiddenContentItems.push({ 
        label: `Hidden Sheet: ${hs.name || 'Unknown'}`, 
        value: hs.state || 'hidden' 
      });
    });
  }
  
  // Embedded objects items
  const embeddedItems = [];
  if (analysis?.detections?.embeddedObjects && Array.isArray(analysis.detections.embeddedObjects)) {
    analysis.detections.embeddedObjects.forEach(eo => {
      const type = eo.type || eo.objectType || 'Object';
      const name = eo.name || eo.filename || '';
      embeddedItems.push({ label: type, value: name });
    });
  }
  ...
  if (analysis?.detections?.macros && Array.isArray(analysis.detections.macros)) {
    analysis.detections.macros.forEach(m => {
      ...
      const location = m.location || m.file || '';
      macroItems.push({ label: type, value: location });
    });
  }
  ...
  if (analysis?.detections?.sensitiveData && Array.isArray(analysis.detections.sensitiveData)) {
    analysis.detections.sensitiveData.forEach(sd => {
      ...
      sensitiveDataItems.push({ 
        label: type, 
        value: preview.length > 30 ? preview.substring(0, 30) + '...' : preview 
      });
    });
  }
  
  // Spelling/grammar items
  ...
  const cleaningSummary = {
    ...
    trackChanges: {
      ...
      scoreImpact: scoreImpacts.trackChanges || 0
    },
    hiddenContent: {
      count: hiddenContentItems.length || fmt(cleaning?.hiddenRemoved) || 0,
      items: hiddenContentItems,
      scoreImpact: scoreImpacts.hiddenContent || 0
    },
    embeddedObjects: {
      count: embeddedItems.length || fmt(cleaning?.mediaDeleted) + fmt(cleaning?.picturesRemoved) || 0,
      items: embeddedItems,
      media: fmt(cleaning?.mediaDeleted) || 0,
      pictures: fmt(cleaning?.picturesRemoved) || 0,
      scoreImpact: scoreImpacts.embeddedObjects || 0
    },
    macros: {
      count: macroItems.length || fmt(cleaning?.macrosRemoved) || 0,
      items: macroItems,
      scoreImpact: scoreImpacts.macros || 0
    },
    sensitiveData: {
      count: sensitiveDataItems.length,
      items: sensitiveDataItems,
      types: Array.from(sensitiveDataTypes),
      scoreImpact: scoreImpacts.sensitiveData || 0
    },
    spellingGrammar: {
      count: allCorrections.length,
      items: spellingItems.slice(0, 10), // Limiter à 10 pour la preview
      scoreImpact: scoreImpacts.spellingGrammar || 0
    }
  };
  ...
  // Ajouter les risques depuis analysis.detections.sensitiveData
  if (analysis?.detections?.sensitiveData && Array.isArray(analysis.detections.sensitiveData)) {
    analysis.detections.sensitiveData.forEach(sd => {
      risksDetected.push({
  ...
  console.log(`[REPORT JSON] Generated: beforeScore=${beforeScore}, afterScore=${afterScore}, improvement=+${afterScore - beforeScore}, corrections=${allCorrections.length}`);
  ...
  return {
    ...
    scoreImprovement: Math.round(afterScore - beforeScore),
    summary: {
...
export function buildReportHtmlDetailed({ 
  ...
  // ============================================================
  // SCORES - Before/After avec fallbacks
  // ============================================================
  const beforeScore = beforeRiskScore ?? analysis?.summary?.beforeRiskScore ?? 50;
  const afterScore = afterRiskScore ?? analysis?.summary?.riskScore ?? Math.min(100, beforeScore + 20);
  ...
  const beforeStyle = getScoreStyle(beforeScore);
  const afterStyle = getScoreStyle(afterScore);
  ...
  if (Array.isArray(spellingErrors) && spellingErrors.length > 0) {
    console.log(`[REPORT HTML] Processing ${spellingErrors.length} spellingErrors`);
  ...
  if (correction?.examples && Array.isArray(correction.examples) && correction.examples.length > 0) {
    console.log(`[REPORT HTML] Processing ${correction.examples.length} correction.examples`);
  ...
  console.log(`[REPORT HTML] Total unique corrections: ${allCorrections.length}`);
  ...
  // ============================================================
  // HTML TEMPLATE COMPLET avec Before/After scores
  // ============================================================
  return `<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    * {
    ...
    .score-box.before {
      ...
      border: 2px solid ${beforeStyle.color}40;
    }
    
    .score-box.after {
      background: ${afterStyle.bg};
      border: 2px solid ${afterStyle.color}40;
    }
    ...
    .score-box.before .score-circle-small {
      background: conic-gradient(${beforeStyle.color} ${beforeScore}%, var(--gray-200) ${beforeScore}%);
      box-shadow: 0 4px 20px ${beforeStyle.color}30;
    }
    
    .score-box.after .score-circle-small {
      background: conic-gradient(${afterStyle.color} ${afterScore}%, var(--gray-200) ${afterScore}%);
      box-shadow: 0 4px 20px ${afterStyle.color}30;
    }
    ...
    .score-box.before .score-number-small { color: ${beforeStyle.color}; }
    .score-box.after .score-number-small { color: ${afterStyle.color}; }
    ...
    .score-box.before .score-box-status {
      background: ${beforeStyle.color}20;
      color: ${beforeStyle.color};
    }
    
    .score-box.after .score-box-status {
      background: ${afterStyle.color}20;
    ...
    @media (max-width: 640px) {
      ...
      .score-improvement-arrow { transform: rotate(90deg); }
...
<body>
  <div class="container">
    ...
    <!-- NOUVEAU: Score Before/After Comparison -->
    <div class="score-comparison-card">
      <div class="score-comparison-title">Security Score Comparison</div>
      <div class="score-comparison">
        <div class="score-box before">
          <div class="score-box-label">Before Cleaning</div>
          <div class="score-circle-small">
            <span class="score-number-small">${Math.round(beforeScore)}</span>
          </div>
          <div class="score-box-status">${beforeStyle.label}</div>
        </div>
        
        ${scoreImprovement > 0 ? `
        <div class="score-improvement">
          <div class="score-improvement-arrow">→</div>
          <div class="score-improvement-value">+${Math.round(scoreImprovement)} pts</div>
        </div>
        ` : ''}
        ...
        <div class="score-box after">
          ...
          <div class="score-circle-small">
            <span class="score-number-small">${Math.round(afterScore)}</span>
          </div>
          <div class="score-box-status">${afterStyle.label}</div>
        </div>
      </div>
