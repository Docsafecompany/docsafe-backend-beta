...
export function buildReportHtmlDetailed({ filename, ext, policy, cleaning, correction, analysis, spellingErrors = [] }) {
  ...
  // ============================================================
  // NOUVEAU: Combiner les corrections de plusieurs sources
  // ============================================================
  const allCorrections = [];
  
  // 1. Corrections de aiCorrectText (format: {before, after})
  if (correction?.examples?.length > 0) {
    correction.examples.forEach(ex => {
      allCorrections.push({
        original: ex.before || ex.original || '',
        corrected: ex.after || ex.corrected || '',
  ...
  // 2. Spelling errors de l'analyse (format: {error, correction})
  if (spellingErrors?.length > 0) {
    spellingErrors.forEach(err => {
      // Éviter les doublons
      const alreadyExists = allCorrections.some(c => 
        c.original === err.error && c.corrected === err.correction
      );
      if (!alreadyExists) {
        allCorrections.push({
          original: err.error || err.word || err.original || '',
          corrected: err.correction || err.suggestion || err.corrected || '',
  ...
  const correctionRows = allCorrections.map(({original, corrected, type, context}) =>
    `<tr>
      <td class="cell-before">
        <div class="code-block">
          <span class="error-text">${escapeHtml(original)}</span>
          ${context ? `<div class="context-text">${escapeHtml(context)}</div>` : ''}
        </div>
      </td>
      <td class="cell-after">
        <div class="code-block">
          <span class="corrected-text">${escapeHtml(corrected)}</span>
          <span class="type-badge ${type}">${formatType(type)}</span>
        </div>
      </td>
    </tr>`
  ).join('');
  
  // Compteur total de corrections
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Corrections Table - NOUVEAU STYLE */
    .corrections-table {
      width: 100%;
...
<body>
  <div class="container">
    ...
    <!-- Text Corrections (COMBINÉ: AI + Spelling Errors) -->
    ${totalCorrections > 0 ? `
    <div class="card">
      <div class="card-header">
        <div class="card-icon green">✏️</div>
        <h2>Text Corrections Applied (${totalCorrections})</h2>
      </div>
...
/**
 * Formater le type d'erreur pour l'affichage
 */
