...
// VERSION 3.4.0 - Universal Risk Model (4 exposure surfaces) + Executive Business Risk (5 categories)
//
// ✅ Keeps: { detections, summary, documentStats } exactly like before.
// ✅ Adds: extractedText (top-level, best-effort) for backend Part 2 Business Risk rules
...
//
// IMPORTANT:
// - This file is detection-only (no advice).
// - If you want Part 2 (Business Risk scoring + recommendations) it should live in server.js,
//   using extractedText + detections + riskObjects/riskSummary.
...
export async function analyzeDocument(fileBuffer, fileType) {
  ...
  let detections;
  let extractedText = ""; // ✅ NEW (best-effort across all types)

  switch (ext) {
    case "docx": {
      const r = await analyzeDOCX(fileBuffer);
      detections = r;
      extractedText = r?.__extractedText || "";
      break;
    }
    case "pptx": {
      const r = await analyzePPTX(fileBuffer);
      detections = r;
      extractedText = r?.__extractedText || "";
      break;
    }
    case "xlsx": {
      const r = await analyzeXLSX(fileBuffer);
      detections = r;
      extractedText = r?.__extractedText || "";
      break;
    }
    case "pdf": {
      const r = await analyzePDF(fileBuffer);
      detections = r;
      extractedText = r?.__extractedText || "";
      break;
    }
  ...
  // ✅ Universal riskObjects + riskSummary (4-surfaces)
  ...
  return {
    ...
    // ✅ NEW: text available for server Part 2 Business Risk logic (non-breaking addition)
    extractedText,
...
async function analyzeDOCX(buffer) {
  ...
  return {
    ...
    // ✅ NEW: delivery/commitment language signals
    businessInconsistencies,

    // ✅ NEW: extracted text for server Part 2 Business Risk rules (internal)
    __extractedText: fullText,
  };
}
...
async function extractPPTXText(zip) {
  ...
  const files = Object.keys(zip.files).filter(
    (k) =>
      (k.startsWith("ppt/slides/slide") || k.startsWith("ppt/notesSlides/")) && k.endsWith(".xml")
  );
...
async function extractXLSXText(zip) {
  ...
  if (!text || text.trim().length < 5) {
    ...
    for (const p of sheetFiles) {
      ...
      const inlineStr = Array.from(xml.matchAll(/<is>[\s\S]*?<\/is>/g)).map((m) =>
        extractTextFromXML(m[0])
      );
...
// ============================================================
// ✅ Delivery & Commitment signals (deterministic)
// ============================================================
...
// ✅ 5 Executive Business Risk categories (aligned with Qualion Clean V1 spec)
...
function buildUniversalRiskOutput({ ext, detections, summary }) {
  ...
  pushFrom(detections.complianceRisks, { surface: SURFACES.visible, source: "complianceRisks" }); // ✅ NEW
...
function mapDetectionToRiskObject(item, meta, ext) {
  ...
  return {
    ...
    riskCategory: riskCategoryKey, // margin / delivery / negotiation / compliance / credibility
    riskCategoryLabel: EXEC_RISK_CATEGORIES[riskCategoryKey] || EXEC_RISK_CATEGORIES.credibility,
...
function mapToExecutiveCategoryAndReason(item, meta) {
  ...
  if (src === "comments" || src === "trackChanges" || src === "hiddenContent") {
    ...
    ruleId =
      src === "comments"
        ? "CREDIBILITY_COMMENTS"
        : src === "trackChanges"
        ? "CREDIBILITY_TRACK_CHANGES"
        : "CREDIBILITY_HIDDEN_CONTENT";
    fixability = "auto-fix";
  }
  ...
  // SensitiveData: map to margin / negotiation / compliance / credibility
  if (src === "sensitiveData") {
    ...
    } else if (cat === "personal" || ["email", "phone", "ssn", "credit_card", "iban"].includes(t)) {
      riskCategoryKey = "compliance";
      ruleId = "COMPLIANCE_PERSONAL_DATA";
    } else {
      riskCategoryKey = "credibility";
  ...
  // Compliance risks => compliance category (aligned with Qualion V1 spec)
  if (src === "complianceRisks") {
    riskCategoryKey = "compliance";
    ruleId = "COMPLIANCE_CONFIDENTIALITY";
    fixability = "manual";
  }
...
function buildNeutralReason(riskCategoryKey, surface, source, item) {
  const catLabel = EXEC_RISK_CATEGORIES[riskCategoryKey] || EXEC_RISK_CATEGORIES.credibility;
  ...
  if (source === "complianceRisks") {
    return `Compliance-related signals detected that require review before external sharing.`;
  }
...
function scoreRiskPoints({ surface, riskCategoryKey, source, item }) {
  ...
  // macros: big trust risk
  if (source === "macros") pts += 3;

  // compliance risks: bump a bit (exec gate)
  if (riskCategoryKey === "compliance") pts += 2;
...
function buildRiskSummary(riskObjects) {
  const byCategory = { margin: 0, delivery: 0, negotiation: 0, compliance: 0, credibility: 0 };
  ...
  for (const ro of riskObjects) {
    ...
    if (s > maxSeverity.score) {
      maxSeverity.score = s;
      maxSeverity.level = s === 4 ? "Critical" : s === 3 ? "High" : s === 2 ? "Medium" : "Low";
    }
  }

  // clientReady rule (strict): Any High/Critical => NO
  ...
  const executiveSignals = [
    ...
    byCategory.compliance > 0 ? EXEC_RISK_CATEGORIES.compliance : null,
