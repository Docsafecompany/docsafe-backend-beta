...
async function analyzeDOCX(buffer) {
  ...
  // Utiliser le nouveau module de détection de données sensibles
  ...
  return {
    // 1️⃣ Sensitive Data (nouveau module amélioré)
...
export function calculateSummary(detections) {
  ...
  // Fonction pour compter par sévérité
  ...
  // ===== CALCUL DU RISK SCORE (inversé: 100 = safe) =====
  // Commencer à 100 (document parfait) et soustraire les pénalités
  let riskScore = 100;
  
  // Pénalités par sévérité
  const penalties = {
    critical: 25,  // Chaque issue critique = -25 points
    high: 15,      // Chaque issue haute = -15 points
    medium: 5,     // Chaque issue moyenne = -5 points
    low: 1         // Chaque issue basse = -1 point
  };
  
  // Pénalités par catégorie (caps pour éviter score négatif)
  ...
  // Appliquer les pénalités par catégorie avec caps
  ...
  // Pénalité additionnelle basée sur la sévérité
  riskScore -= criticalIssues * 10;
  riskScore -= highIssues * 5;
  
  // Pénalité pour volume total
  ...
  if (categoryCounts.macros > 0) {
    recommendations.push({
      priority: 'critical',
      message: 'Remove VBA macros before sharing - they can contain malicious code',
      category: 'macros'
    });
  }
  ...
  if (categoryCounts.complianceRisks > 0) {
    recommendations.push({
      priority: 'high',
      message: 'Document may contain GDPR-regulated personal data - review before sharing',
  ...
  if (categoryCounts.metadata > 0) {
    recommendations.push({
      priority: 'medium',
      message: 'Clean document metadata to remove author names and company information',
      category: 'metadata'
    });
  }
  ...
  if (categoryCounts.excelHiddenData > 0) {
    recommendations.push({
      priority: 'high',
      message: 'Hidden sheets, rows or columns detected - may expose confidential data',
      category: 'excelHiddenData'
    });
  }
  
  // Log pour debug
  console.log(`[RISK SCORE] Calculated: ${riskScore} (total issues: ${totalIssues}, breakdown: {
