...
// VERSION 3.2.1 - Enterprise-grade avec 9 catégories de détection
// PATCH 3.2.1:
// - PPTX: extract text inside shapes (ovals/rectangles/etc.) -> visualObjects.type="shape_text"
// - DOCX: extract text inside textboxes -> visualObjects.type="shape_text"
// - Sensitive data: keep FULL value in `value` + optional `maskedValue`
// - DOCX comments: safer text extraction from parsed XML nodes
...
async function analyzeDOCX(buffer) {
  ...
  return {
    // 1️⃣ Sensitive Data
    // IMPORTANT: FULL value in `value`. If masking exists, also keep `maskedValue`.
...
function extractTextFromParsedXmlNode(node) {
  ...
  if (typeof node === "object") {
    let out = "";
    // common xml2js pattern: text node in "_" or in "w:t"
    ...
    try {
      ...
      if (parsed?.["w:comments"]?.["w:comment"]) {
        ...
        commentsList.forEach((comment, index) => {
          ...
          // safer text extraction (instead of JSON.stringify)
    ...
    if (file) {
      ...
      if (noteText && noteText.trim().length > 5) {
        ...
        comments.push({
          ...
          text:
            noteText.trim().substring(0, 300) + (noteText.length > 300 ? "..." : ""),
          date: null,
...
async function analyzeVisualObjects(zip, fileType) {
  ...
  if (fileType === "pptx") {
    ...
    for (const slideFile of slideFiles) {
      ...
      for (const m of shapeMatches) {
        ...
        // A) SHAPE TEXT (the main thing you want)
        const cleanText = (text || "").trim();
        ...
        // B) Potential covering shapes (solid fill, no text, large)
      ...
      // Keep a summary item if you want (optional)
  ...
  if (fileType === "docx") {
    ...
    if (documentXml) {
      ...
      // 1) Text inside textboxes (w:txbxContent)
      ...
      // 2) Your previous “covering shapes” heuristic
