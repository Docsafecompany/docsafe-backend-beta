...
// ============= SUMMARY CALCULATION =============
// IMPORTANT: Score 100 = PARFAIT (safe), Score 0 = DANGEREUX (critical)
...
export function calculateSummary(detections) {
  ...
  // Pondération des différents types de problèmes
  const weights = {
    metadata: 2,
    comments: 3,
    trackChanges: 2,
    ...
    sensitiveFormulas: 6,
    embeddedObjects: 3,
    macros: 10, // Très critique !
    sensitiveData: 6,
    spellingErrors: 1, // Faible impact sécurité
    brokenLinks: 2,
    complianceRisks: 8
  };
  ...
  // Score 100 = parfait, score 0 = très dangereux
  // On part de 100 et on soustrait selon les problèmes trouvés
  let riskScore = Math.max(0, Math.min(100, 100 - weightedScore));
  
  // Ajustements basés sur les problèmes critiques
