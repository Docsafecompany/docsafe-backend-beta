...
// VERSION 3.2.2 - Enterprise-grade avec 9 catégories de détection
// PATCH 3.2.2:
// - Extraction texte "FULL COVERAGE" (DOCX headers/footers/footnotes/endnotes/textboxes, PPTX slides+notes)
// - XLSX fallback si sharedStrings vide (lecture basique des sheet xml)
// - Conserve patchs 3.2.1: shapes, textboxes, full sensitive value, comments safer
//
// PATCH 3.2.2a (IMPORTANT):
// ✅ Fix ROOT CAUSE des mots collés (runs XML collés => "Impactof") :
//    - Remplace extractTextFromXML() par une version token-based + séparateurs de paragraphes/line breaks
//    - Ajoute une règle "safe space between tokens" (conservatrice)
//
// PATCH 3.2.3 (NEW):
// ✅ Ajoute riskObjects[] + riskSummary (Universal Model 4 surfaces)
// ✅ Ne casse PAS detections (backward compatible)
...
export async function analyzeDocument(fileBuffer, fileType) {
  ...
  // ✅ Calculer les stats du document (pages/slides/sheets/tables)
  ...
  // ✅ NEW: Universal Risk Objects (4 surfaces) — sans casser detections
  const riskObjects = buildRiskObjects({ fileExt: ext, detections });
...
async function computeDocumentStats(fileBuffer, ext) {
  ...
  try {
    if (ext === "docx") {
      ...
      if (docXml) {
        // Compter les sections/pages (approximation)
    ...
    // Fail-safe: ne casse jamais l'analyse
...
async function extractDOCXText(zip) {
  ...
  for (const p of targets) {
    ...
    // BONUS: capture textbox content (w:txbxContent) specifically (already in xml, but reinforced)
    const tbxMatches = xml.matchAll(
...
async function extractXLSXText(zip) {
  ...
  // fallback: read sheet xml values (basic, but helps when sharedStrings missing)
  if (!text || text.trim().length < 5) {
    ...
    for (const p of sheetFiles) {
      ...
      if (!f) continue;
      const xml = await f.async("text");
      // capture inlineStr + v (values)
      const inlineStr = Array.from(xml.matchAll(/<is>[\s\S]*?<\/is>/g)).map(
        (m) => extractTextFromXML(m[0])
      );
      const values = Array.from(xml.matchAll(/<v>([\s\S]*?)<\/v>/g)).map(
        (m) => (m[1] || "").trim()
      );
...
function extractTextFromParsedXmlNode(node) {
  ...
  if (Array.isArray(node))
    return node.map(extractTextFromParsedXmlNode).join(" ");
...
// ============================================================
// ============= VISUAL OBJECTS ANALYSIS (PATCHED) =============
// ============================================================
...
async function analyzeVisualObjects(zip, fileType) {
  ...
  if (fileType === "pptx") {
    ...
    for (const slideFile of slideFiles) {
      ...
      for (const m of shapeMatches) {
        ...
        const hasTextBody =
          shapeXml.includes("<p:txBody") || shapeXml.includes("<a:t>");
        const text = hasTextBody ? extractTextFromXML(shapeXml) : "";

        const cleanText = (text || "").trim();
        if (cleanText.length > 0) {
          visualObjects.push({
            id: `visual_shape_text_slide${slideNum}_${
              shapeId || visualObjects.length
            }_${Date.now()}`,
            type: "shape_text",
            description: `Text inside shape${geom ? ` (${geom})` : ""}`,
            text: cleanText,
            location: `${slideLocation}${shapeName ? ` — ${shapeName}` : ""}${
              shapeId ? ` (id ${shapeId})` : ""
            }`,
        ...
        if (hasSolidFill && (!cleanText || cleanText.length === 0) && ext) {
          if (ext.cx > 2000000 && ext.cy > 500000) {
            ...
            visualObjects.push({
              id: `visual_covering_slide${slideNum}_${
                shapeId || visualObjects.length
              }_${Date.now()}`,
              type: "shape_covering_text",
              description: `Large solid shape may cover content${
                geom ? ` (${geom})` : ""
              }`,
              location: `${slideLocation}${shapeName ? ` — ${shapeName}` : ""}${
                shapeId ? ` (id ${shapeId})` : ""
  ...
  if (fileType === "docx") {
    ...
    if (documentXml) {
      ...
      const tbxMatches = content.matchAll(
        /<w:txbxContent\b[^>]*>[\s\S]*?<\/w:txbxContent>/g
      );
      let tbxIndex = 0;
...
async function analyzeComplianceRisks(text) {
  ...
  const gdprPatterns = [
    {
      pattern: /numéro de sécurité sociale|social security|ssn/i,
      risk: "Social Security Number reference",
      severity: "critical",
    },
    {
      pattern: /date de naissance|birth date|dob|née? le/i,
    ...
    {
      pattern: /carte d'identité|identity card|id card|cni/i,
      risk: "ID card reference",
      severity: "high",
    },
    {
      pattern: /numéro de permis|driver'?s? license|permis de conduire/i,
      risk: "Driver license reference",
      severity: "high",
    },
    {
      pattern: /données de santé|health data|medical record|dossier médical/i,
...
// --- NEW helpers for safe token spacing ---
...
// conservateur : ajoute un espace entre 2 tokens si ça ressemble à "Impact"+"of" (runs collés)
// sans casser "U.S.", "e.g.", "3.14", "C++", etc.
function shouldInsertSpace(prevText, nextToken) {
  ...
  // on prend le dernier char non-espace du prev
  ...
  // Ne pas insérer dans nombres décimaux: "3" + ".14"
  if (isDigit(p) && next.startsWith(".")) return false;

  // Ne pas insérer autour d'un point "attaché"
  if (p === "." || n === ".") return false;

  // Cas run split de mots
...
/**
 * ✅ PATCH: extraction token-based + séparateurs
 * - transforme certains tags en espaces / retours ligne
 * - extrait le texte en "tokens" dans l’ordre
 * - reconstruit avec espaces safe entre runs
 */
function extractTextFromXML(xml) {
  const raw = String(xml || "");
  if (!raw.trim()) return "";

  // 1) Normaliser tags qui représentent des séparateurs (paragraphes/line breaks/tabs)
  // DOCX
  let x = raw
    .replace(/<w:tab\b[^\/>]*\/>/gi, " \t ")
    .replace(/<w:br\b[^\/>]*\/>/gi, " \n ")
  ...
  // 2) Extraire tout ce qui est entre tags (texte brut) en conservant ordre
  const tokens = [];
  const re = />([^<]+)</g;
  let m;
  while ((m = re.exec(x)) !== null) {
    ...
    // On split en gardant les séparateurs \n et \t
    const parts = t.split(/(\n|\t)/g);
  ...
  // 4) Cleanup final (respecte paragraphes)
  out = out
    .replace(/[ \t]+\n/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
    .replace(/[ \t]{2,}/g, " ")
...
function determineSeverity(text) {
  ...
  if (
    ...
    return "high";
  if (
    /todo|fixme|draft|preliminary|internal|review|brouillon|à revoir/i.test(lower)
  )
    return "medium";
  return "low";
}
...
export function calculateSummary(detections) {
  ...
  if (categoryCounts.hiddenContent > 0) {
    recommendations.push({
      priority: "high",
      message: "Hidden content detected - could contain sensitive information",
  ...
  return {
    ...
    hasHiddenContent:
      categoryCounts.hiddenContent > 0 || categoryCounts.excelHiddenData > 0,
...
// ============================================================
// ============= UNIVERSAL RISK OBJECTS (4 SURFACES) =============
// ============================================================

function surfaceForDetectionKey(key) {
  // mapping simple et stable (tu pourras affiner après)
  if (key === "metadata") return "Residual";
  if (
    key === "comments" ||
    key === "trackChanges" ||
    key === "hiddenContent" ||
    ...
    return "Hidden";
  if (key === "embeddedObjects" || key === "macros") return "Structural";
  if (
    key === "visualObjects" ||
    key === "spellingErrors" ||
    key === "orphanData" ||
    ...
    return "Visible";
  if (key === "sensitiveData" || key === "complianceRisks") return "Visible";
  return "Hidden";
}

function riskTypeForKey(key) {
  // types universels (pas “par format”)
  const map = {
    metadata: "Residual / Metadata",
    comments: "Hidden Content",
    trackChanges: "Hidden Content",
    hiddenContent: "Hidden Content",
    excelHiddenData: "Hidden Content",
    embeddedObjects: "Structural Logic",
    macros: "Structural Logic",
    brokenLinks: "Residual / Metadata",
    orphanData: "Residual / Metadata",
    visualObjects: "Visible Content",
    spellingErrors: "Visible Content",
    sensitiveData: "Visible Content",
    complianceRisks: "Visible Content",
  };
  return map[key] || "Hidden Content";
}

function buildRiskObject({ fileExt, detectionKey, item }) {
  // scoring universel (ton modèle), cheap & stable
  const points =
    3 + // Hidden but accessible (base)
    (detectionKey === "embeddedObjects" || detectionKey === "macros" ? 2 : 0) + // generated/derived-ish
    (detectionKey === "embeddedObjects" ? 2 : 0) + // cross-object dependency
    (detectionKey === "metadata" ? 2 : 0) + // metadata revealing intent
    (detectionKey === "sensitiveData" || detectionKey === "complianceRisks"
      ? 2
      : 0); // financial/strategic proximity

  // seuils universels
  const severity = points >= 7 ? "high" : points >= 4 ? "medium" : "low";

  const surface = surfaceForDetectionKey(detectionKey);

  // phrase user-facing, pas de mécanique
  const reason =
    item?.description ||
    item?.reason ||
    item?.text ||
    item?.value ||
    `${detectionKey} detected`;

  return {
    id: item?.id || `${detectionKey}_${Date.now()}`,
    type: riskTypeForKey(detectionKey),
    surface, // Visible | Hidden | Structural | Residual
    file: fileExt, // docx/pptx/xlsx/pdf
    severity, // low/medium/high
    score: points, // points bruts (universal)
    reason: String(reason).slice(0, 240),
    affectedArea: item?.location || null,
    source: {
      category: detectionKey,
      sourceId: item?.id || null,
    },
  };
}

function buildRiskObjects({ fileExt, detections }) {
  const keys = Object.keys(detections || {});
  const out = [];

  for (const key of keys) {
    const arr = detections?.[key];
    if (!Array.isArray(arr) || arr.length === 0) continue;

    for (const item of arr) {
      out.push(buildRiskObject({ fileExt, detectionKey: key, item }));
    }
  }
...
function buildRiskSummary(riskObjects) {
  const summary = {
    total: riskObjects.length,
    bySurface: { Visible: 0, Hidden: 0, Structural: 0, Residual: 0 },
    bySeverity: { low: 0, medium: 0, high: 0, critical: 0 },
  };

  for (const r of riskObjects) {
    if (summary.bySurface[r.surface] !== undefined)
      summary.bySurface[r.surface] += 1;
    const sev = (r.severity || "low").toLowerCase();
    if (summary.bySeverity[sev] !== undefined) summary.bySeverity[sev] += 1;
  }

  // index simple (peut être aligné ensuite avec server risk score)
  const riskIndex = riskObjects.reduce((acc, r) => acc + (r.score || 0), 0);
  ...
  return { ...summary, riskIndex, level };
}
