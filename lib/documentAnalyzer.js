...
// VERSION 3.1 - Enterprise-grade avec 9 catégories de détection
...
async function analyzeDOCXHiddenContentEnriched(zip) {
  ...
  if (documentXml) {
    ...
    const whiteTextMatches = [
      ...content.matchAll(
        /<w:color[^>]*w:val="(FFFFFF|ffffff|white)"[^>]*\/>/gi
      ),
    ];
    ...
    const smallFontMatches = [
      ...content.matchAll(/<w:sz[^>]*w:val="([1-9])"[^>]*\/>/g),
    ];
...
async function analyzePPTXHiddenContentEnriched(zip) {
  ...
  for (const slideFile of slideFiles) {
    ...
    if (file) {
      ...
      // White text
      const whiteTextCount =
        content.match(/<a:srgbClr val="(FFFFFF|ffffff)"/g)?.length || 0;
      if (whiteTextCount > 2) {
        hidden.push({
