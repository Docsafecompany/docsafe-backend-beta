...
function shouldInsertSpace(prev, next) {
  ...
  // cas lettre + lettre : très probable run split => espace
  const isLetter = (ch) => /[A-Za-zÀ-ÖØ-öø-ÿ]/.test(ch);
  const isDigit = (ch) => /[0-9]/.test(ch);

  // ne pas coller/espacer autour des ponctuations “attachées”
  ...
  // Ne pas insérer dans abréviation type "U" + "." ou "." + "S"
  if (p === "." || n === ".") return false;

  // Cas run split de mots
  if (isLetter(p) && isLetter(n)) return true;

  // Cas lettre + chiffre (ex: "ISO" + "27001") => souvent voulu sans espace ? en vrai on veut "ISO 27001"
  if (isLetter(p) && isDigit(n)) return true;

  // Cas chiffre + lettre (ex: "20" + "mg") => "20 mg"
...
function extractTextFromXML(xml) {
  ...
  // Exemple: <w:t>Impact</w:t><w:t>of</w:t> => ["Impact","of"]
  const tokens = [];
  const re = />([^<]+)</g;
  let m;
  ...
  // 3) Reconstruire avec insertion d'espaces "safe" entre tokens
  let out = tokens[0];
  for (let i = 1; i < tokens.length; i++) {
    const prev = out;
    const next = tokens[i];

    // Si le token contient déjà \n ou \t, on garde des séparateurs propres
    // (mais tokens ici sont trim, donc pas de \n dans tokens; séparateurs sont générés via tags => captés en tokens?)
    // On gère explicitement si next est un séparateur symbolique (rare ici).
    if (next === "\n") {
      out += "\n";
      continue;
    }
    ...
    out += (shouldInsertSpace(prev, next) ? " " : " ") + next;
  }
