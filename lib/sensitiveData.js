...
const PATTERNS = {
  ...
  // ✅ CORRIGÉ: Capture "500$", "4500$", "4 500€", "12,500.00$"
  priceEuro: /(\d{1,3}(?:[\s.,]\d{3})*|\d+)(?:[.,]\d{1,2})?\s*[€$£¥₹]/gi,
  priceSymbolFirst: /[€$£¥₹]\s*(\d{1,3}(?:[\s.,]\d{3})*|\d+)(?:[.,]\d{1,2})?/gi,
  priceWithCurrency: /(\d{1,3}(?:[\s.,]\d{3})*|\d+)(?:[.,]\d{1,2})?\s*(EUR|USD|GBP|CHF|JPY|euros?|dollars?|francs?)/gi,
