...
export async function cleanPDF(inputBuffer, { pdfMode = "sanitize", extractTextFn } = {}) {
  ...
  for (const page of pdf.getPages()) {
    ...
    if (a && a.asArray) annotsBefore += a.asArray().length;
  }
  ...
  if (Names) {
    ...
    if (maybeEF) {
      const arr = maybeEF.lookupMaybe(PDFName.of('Names'));
      if (arr && arr.asArray) stats.embeddedFilesRemoved = arr.asArray().length / 2; // pairs [name, dict]
      Names.set(PDFName.of('EmbeddedFiles'), pdf.context.obj({ Names: [] }));
    }
  }

  return { outBuffer: Buffer.from(await pdf.save()), stats };
}
